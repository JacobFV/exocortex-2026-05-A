...
The implemented foundation includes EventGraph-backed continuity, configurable graph context assembly for model turns, evaluation/self-modification objects and evaluation suites, simulation/retry records, guarded self-modification promotion, Electron and Expo host shells, persisted Electron sessions/artifact blobs, artifact integrity/retention/repair, serial transport health, passing OpenAI-compatible live model checks, Electron continuous STT bridging, host media modalities, browser frame artifacts, media provider/playback wiring, calibration acceptance, safety denial audit state, pre-execution actuator approvals, hardware bench smoke tooling, SQLite migrations, and filtered continuity exports.

Remaining work is tracked in `docs/remaining-work.md`. The most important next steps are adding a real Electron renderer bundle with browser-level tests, building Expo media action/observation parity, wiring automated evaluation-suite execution, and validating one physical ESP bridge end to end.
